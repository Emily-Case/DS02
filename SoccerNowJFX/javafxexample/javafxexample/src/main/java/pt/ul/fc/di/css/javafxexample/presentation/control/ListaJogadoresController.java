...
public class ListaJogadoresController {
    ...
    public void initialize() {
        try {
            //Buscar jogadores do servidor
            List<Jogador> jogadores = connectionController.getAllJogadores();

            //Transformar a lista de jogadores em uma lista de strings (nomes, por exemplo)
            ObservableList<String> jogadoresStr = FXCollections.observableArrayList();
            for (Jogador jogador : jogadores) {
                jogadoresStr.add(jogador.toString());
            }

            //Preencher a ListView
            listaJogadores.setItems(jogadoresStr);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
